...
public abstract class Schema implements Serializable {
   ...
   * used to load the generic properties of all types of schemas like {@code title} or {@code
   * description}.
   *
   ...
   * Constructor.
   *
   * @param builder the builder containing the optional title, description and id attributes of the
   *     schema
   */
   ...
   * be queried.
   *
   * <p>For example the field {@code "#/rectangle/a"} is defined by the following schema:
   *
   * <pre>
   * <code>
  ...
  public int hashCode() {
    return Objects.hash(
        title, description, id, defaultValue, nullable, readOnly, writeOnly, unprocessedProperties);
  }
   ...
   *
   * <p>First it adds the {@code "title} , {@code "description"} and {@code "id"} properties then
   * calls {@link #describePropertiesTo(JSONPrinter)}, which will add the subclass-specific
   * properties.
   *
   * <p>It is used by {@link #toString()} to serialize the schema instance into its JSON
   * representation.
   *
   ...
   * This method is called by {@link #describeTo(JSONPrinter)} after adding the generic properties
   * if they are present ({@code id}, {@code title} and {@code description}). As a side effect,
  ...
  protected ValidationException failure(Class<?> expectedType, Object actualValue) {
    return new ValidationException(
        this, expectedType, actualValue, "type", schemaLocation.toString());
  }
   ...
   *
   * <p>http://www.artima.com/lejava/articles/equality.html
   *
